...
function tecRowHTML(fila) {
  return (
    ...
    fila.id +
    '">' +
    (tecPlano() ? '<td class="tec-cat">' + esc(fila.categoria || "Sin categoría") + "</td>" : "") +
    '<td class="tec-curso"><input type="text" data-tec-id="' +
    fila.id +
    '" data-tec-field="curso" value="' +
    esc(fila.curso) +
    '"></td><td class="tec-pub"><input type="text" class="' +
    (tecFechaMala(fila.publicacion) ? "tec-fecha-mala" : "") +
    '" data-tec-id="' +
    fila.id +
    '" data-tec-field="publicacion" value="' +
    esc(tecFechaVer(fila.publicacion)) +
    '" placeholder="DD/MM/AAAA"' +
    (tecFechaMala(fila.publicacion) ? ' title="Esta fecha no se entiende. Escribila como DD/MM/AAAA."' : "") +
    "></td><td class=\"tec-chk\">" +
...
function tecFiltrando() {
  return !!(state.tecFiltro || state.tecDiseno || state.tecPendiente || state.tecOrden);
}
...
document.addEventListener("click", (ev) => {
  ...
  switch (val9) {
    ...
    case "tec:del":
      ...
      break;
    case "tec:add": {
      // Desde la fila de un grupo ya viene la categoría; desde el botón de
      // arriba hay que preguntarla, porque en modo ordenado no hay grupos.
      let catTec = el.dataset.cat;
      if (catTec === undefined) {
        catTec = (prompt("¿En qué categoría? (podés dejarlo vacío)") || "").trim();
      }
      const nombreTec = (prompt("Nombre del curso") || "").trim();
      if (!nombreTec) break;
      (state.tecnico.push({
        id: uid(),
        categoria: catTec,
