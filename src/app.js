...
function mapaSectorMap() {
  ...
  return (
    '<div class="mapa-sectormap">\n      <div class="mapa-sectormap-t" style="display:flex;align-items:center;gap:8px">Distribución por sector' +
    (filtroActivo
...
function tecRows() {
  ...
  if (!filtro) return state.tecnico;
  return state.tecnico.filter(
    (fila) => fila.curso.toLowerCase().includes(filtro) || (fila.categoria || "").toLowerCase().includes(filtro),
  );
}
function tecGroups() {
  const orden = [],
    grupos = {};
  tecRows().forEach((fila) => {
...
function tecRowHTML(fila) {
  return (
    ...
    esc(fila.curso) +
    '"></td><td class="tec-pub"><input type="text" data-tec-id="' +
    fila.id +
    '" data-tec-field="publicacion" value="' +
    esc(fila.publicacion) +
    '" placeholder="AAAA-MM-DD"></td><td class="tec-chk">' +
...
function tecGroupHTML(grupo) {
  return (
    '<tbody class="tec-group"><tr class="tec-cat-row"><td colspan="10">' +
    esc(grupo.categoria) +
    ' <span class="tec-cat-n">' +
    grupo.filas.length +
    "</span></td></tr>" +
    grupo.filas.map(tecRowHTML).join("") +
    '<tr class="tec-add-row"><td colspan="10"><button class="btn btn-ghost btn-sm" data-action="tec:add" data-cat="' +
    esc(grupo.categoria) +
    '">+ Agregar curso a ' +
    esc(grupo.categoria) +
    "</button></td></tr></tbody>"
...
const TEC_HEAD =
  '<thead><tr><th>Curso</th><th>Publicación</th><th>Portada</th><th>Mosaico</th><th>Evaluación</th><th>Textos</th><th>Diseño</th><th>Estado / Comentario</th><th>Mapa</th><th></th></tr></thead>';
function tecListHTML() {
  const grupos = tecGroups();
  if (!grupos.length)
    return '<div class="empty"><div class="big">🔍</div><div style="font-weight:700;color:var(--ink)">Sin resultados</div><div style="margin-top:4px">Probá con otra búsqueda.</div></div>';
  return (
    '<div class="tec-table-wrap"><table class="tec-table">' + TEC_HEAD + grupos.map(tecGroupHTML).join("") + "</table></div>"
  );
}
function renderTecList() {
  const el = $("#tecList");
  if (el) el.innerHTML = tecListHTML();
}
function tecKpi(n, label, color) {
  return (
    '<div class="kpi" style="--kpi:' + color + '"><div class="kpi-num">' + n + '</div><div class="kpi-lbl">' +
    esc(label) +
    "</div></div>"
  );
}
function tecKpisHTML() {
  const lista = state.tecnico,
    total = lista.length,
    pendientes = lista.filter((f) => f.diseno === "X" || f.diseno === "Rediseñando").length,
    conObs = lista.filter((f) => f.estado && f.estado.trim() && f.estado.trim().toLowerCase() !== "ok").length,
    vinculados = lista.filter((f) => f.cardId).length;
  return (
    '<div class="res-grid">' +
    tecKpi(total, "Cursos en seguimiento", "var(--coto-blue)") +
    tecKpi(pendientes, "Sin diseño actualizado", "var(--bad)") +
    tecKpi(conObs, "Con observaciones", "var(--warn)") +
    tecKpi(vinculados, "Vinculados al Mapa", "var(--ok)") +
    "</div>"
  );
}
  ...
  const lista2 = boardCards(),
    ...
    lista = [
      ...
      },
      {
        ...
        cat: "sistemas",
        ic: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8.5"/><circle cx="12" cy="12" r="3"/></svg>',
        t: "Mi semana",
      ...
      },
      {
        ...
        ic: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="8" r="3"/><path d="M3.5 20c0-3.5 2.5-6 5.5-6s5.5 2.5 5.5 6"/><circle cx="17" cy="9" r="2.5"/><path d="M14.5 20c.3-2.7 1.8-4.6 4-5.3"/></svg>',
        t: "Carga del equipo",
...
document.addEventListener("click", (ev) => {
  ...
  switch (val9) {
    ...
    case "tec:del":
      ...
      break;
    case "tec:add": {
      const catTec = el.dataset.cat || "",
        nombreTec = (prompt("Nombre del curso") || "").trim();
      if (!nombreTec) break;
      (state.tecnico.push({
        id: uid(),
        categoria: catTec,
